...
{
	o->prim.plane.origin = !ft_strncmp(n, "origin", 6) ?
							get_float3(val) : o->prim.plane.origin;
	if (o->type == plane)
	{
		o->prim.plane.normal = !ft_strncmp(n, "normal", 6) ?
							get_float3(val) : o->prim.plane.normal;
	}
	else if (o->type == sphere)
	{
		o->prim.sphere.radius = !ft_strncmp(n, "radius", 6) ?
					get_number(val) : o->prim.sphere.radius;
		o->prim.sphere.r2 = o->prim.sphere.radius * o->prim.sphere.radius;
	}
	else if (o->type == cylinder)
	{
		o->prim.cylinder.normal = !ft_strncmp(n, "normal", 6) ?
							get_float3(val) : o->prim.cylinder.normal;
		o->prim.cylinder.radius = !ft_strncmp(n, "radius", 6) ?
							get_number(val) : o->prim.cylinder.radius;
		o->prim.cylinder.r2 = o->prim.cylinder.radius * o->prim.cylinder.radius;
		o->prim.cylinder.height = !ft_strncmp(n, "height", 6) ?
							get_number(val) : o->prim.cylinder.height;
	}
}

void	get_primitives_details6(t_object *o, char *n,
									struct _json_value *val)
{
	if (o->type == bool_substraction || o->type == bool_intersection)
	{
		o->prim.sphere.origin = !ft_strncmp(n, "origin", 6) ?
					get_float3(val) : o->prim.sphere.origin;
...
{
	o->type = !ft_strcmp(n, "type") ?
		get_type(val) : o->type;
	o->material = !ft_strcmp(n, "material") ?
			get_material(val) : o->material;
	o->roughness = !ft_strcmp(n, "roughness") ?
				get_number(val) : o->roughness;
